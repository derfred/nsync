...
class PatternMatch:
  ...
  def __init__(self, start_index: int, end_index: int, start_time: float, end_time: float, matched_events: List[Dict], neuron_mapping: Dict):
...
class PatternMatcher:
    ...
    def find_matches(self, events: List[Dict], pattern: EventPattern, start_time: Optional[float] = None, end_time: Optional[float] = None) -> List[PatternMatch]:
      ...
      for start_idx in range(len(filtered_events)):
        if match := self._try_match_at_position(filtered_events, pattern, start_idx):
          matches.append(match)

      return self._deduplicate_matches(matches)

    def _deduplicate_matches(self, matches: List[PatternMatch]) -> List[PatternMatch]:
      """Remove duplicate matches that have identical event sequences.

      Two matches are considered duplicates if they have the same start_time,
      end_time, and sequence of matched events.
      """
      if not matches:
        return matches
      ...
      for match in matches:
        # Create a signature based on the actual matched events
        signature = (
            match.start_time,
            match.end_time,
            tuple(
                (event['time'], event['type'], tuple(sorted(event['neurons'])))
                for event in match.matched_events
            )
        )
    ...
    def _filter_events_by_time(self, events: List[Dict], start_time: Optional[float], end_time: Optional[float]) -> List[Dict]:
    ...
    def _try_match_at_position(self, events: List[Dict], pattern: EventPattern, start_idx: int) -> Optional[PatternMatch]:
      ...
      for pattern_step in pattern.steps:
        match_result = self._find_next_matching_event(events, current_idx, pattern_step, neuron_mapping)
    ...
    def _find_next_matching_event(self, events: List[Dict], start_idx: int, pattern_step: Dict, neuron_mapping: Dict) -> Optional[Tuple]:
      ...
      for i in range(start_idx, len(events)):
        ...
        neurons_match, updated_mapping = self._matches_neurons(event['neurons'], pattern_step.get('neurons'), neuron_mapping)
    ...
    def _match_wildcard(self, events: List[Dict], start_idx: int, pattern_step: Dict, neuron_mapping: Dict) -> Optional[Tuple]:
    ...
    def _matches_neurons(self, event_neurons: List[int], neuron_spec: Optional[Dict], current_mapping: Dict) -> Tuple[bool, Dict]:
    ...
    def _matches_neuron_groups(self, event_neurons: List[int], group_sizes: List[int], current_mapping: Dict) -> Tuple[bool, Dict]:
...
def apply_pattern(pattern: EventPattern, result, start_time: Optional[float] = None, end_time: Optional[float] = None) -> List[PatternMatch]:
...
def apply_pattern_to_multiple(pattern: EventPattern, results: List, start_time: Optional[float] = None, end_time: Optional[float] = None) -> Dict[int, List[PatternMatch]]:
  ...
  for i, result in enumerate(results):
    all_matches[i] = apply_pattern(pattern, result, start_time, end_time)
  return all_matches
